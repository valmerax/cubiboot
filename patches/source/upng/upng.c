...
extern void gmfree(void *memory);
...
static void upng_free_source(upng_t* upng)
{
	if (upng->source.owning != 0) {
		gmfree((void*)upng->source.buffer);
	}
...
upng_error upng_decode(upng_t* upng)
{
	...
	compressed = (unsigned char*)gmalloc(compressed_size);
	if (compressed == NULL) {
	...
	inflated = (unsigned char*)gmalloc(inflated_size);
	if (inflated == NULL) {
		gmfree(compressed);
		SET_ERROR(upng, UPNG_ENOMEM);
		return upng->error;
	} else {
		memset(inflated, 0, inflated_size);
	}
	...
	upng->buffer = (unsigned char*)gmalloc(upng->size);
	if (upng->buffer == NULL) {
		gmfree(inflated);
		upng->size = 0;
		SET_ERROR(upng, UPNG_ENOMEM);
		...
		memset(upng->buffer, 0, upng->size);
	}
...
static upng_t* upng_new(void)
{
	...
	upng = (upng_t*)gmalloc(sizeof(upng_t));
	if (upng == NULL) {
		return NULL;
	}
